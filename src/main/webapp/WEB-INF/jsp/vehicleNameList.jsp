...
<jsp:include page="shared/header.jsp">
    <jsp:param name="pageName" value="vehicleNameList"/>
</jsp:include>
<div id="main" class="card-body">
    ...
    <form:form id="searchForm" modelAttribute="searchCommand">
        ...
        <div class="row">
            ...
            <div class="form-group col-md-2">
                <a href="/vehicleNameList.html?all" class="btn btn-success">
    ...
    <c:if test="${not empty accessoryListPage.content}">
        ...
        <table class="table table-striped">
            ...
            <tbody>
            <c:forEach items="${accessoryListPage.content}" var="vehicleName">
                <tr>
                    <td>
                        <security:authorize access="isAuthenticated()">
                            <a href="?id=${vehicleName.id}">${vehicleName.id}</a>
                        </security:authorize>
                        <security:authorize access="isAnonymous()">
                            ${vehicleName.id}
                        </security:authorize>
                    </td>
                    <td>${empty vehicleName.name?'Brak danych': vehicleName.name}</td>
                    <security:authorize access="hasRole('ADMIN')">
                        <td>
                            <c:url var="deleteUrl" value="/vehicleNameList.html?did=${vehicleName.id}&page=${accessoryListPage.number}&size=${accessoryListPage.size}" />
                            <c:url var="editUrl" value="/vehicleNameForm.html?id=${vehicleName.id}" />
        ...
        <c:set var="mainUrl" value="vehicleNameList.html" scope="request"/>
